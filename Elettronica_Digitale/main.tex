...
\begin{definizione}
	[Formula generale per il numero di MOS di una rete]
	\[
...
\subsubsection{Costo in termini di area}
\subsubsection{Costo in termini di tempo di commutazione}
Nel caso peggiore le nostre porte \textbf{devono} avere tempo di commutazione uguale o inferiore a quello dell'inverter.
\subsubsection{Stima dei costi per NAND e NOR}
\paragraph{NOR}
$\s$
\begin{figure}[H]
	\centering
	\includegraphics[width=0.25\linewidth]{"img/schema rete nor"}
	\caption{Schema rete NOR}
\end{figure}
Per quanto riguarda la PUN, dove c'è una serie, abbiamo $\left(\frac{W}{L}\right)_{1,2,3,4} = x$, per cui:
$$
\frac{1}{x} + \frac{1}{x} + \frac{1}{x} + \frac{1}{x} = \frac{1}{p} \to 4p = 20
$$
Per quanto riguarda la PDN, dove c'è un parallelo, abbiamo $\left(\frac{W}{L}\right)_{5,6,7,8} = y$, per cui:
$$
...
$\s$
\begin{center}
	\includegraphics[width=0.3\linewidth]{"img/schema rete nand"}
\end{center}
Per la PUN troviamo facilmente che:
$$
\left(\frac{W}{L}\right)_{1,2,3,4} = p = 5
$$
Per la PDN abbiamo $\left(\frac{W}{L}\right)_{5,6,7,8} = x$, per cui:
$$
